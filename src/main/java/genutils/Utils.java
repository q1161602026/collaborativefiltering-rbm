...
public class Utils {
     ...
     * matrix
     * @param m is in the form [0 0 5 4 2 0 0 3 ... 0 0 1 2 5]
     * @return [0 0 1 1 1 0 0 1 ... 0 0 1 1 1]
     */
    public static DoubleMatrix getBinaryMatrix(DoubleMatrix m) {
        DoubleMatrix binaryMatrix = DoubleMatrix.zeros(m.getRows(), m.getColumns());
        for(int i = 0; i < m.getRows(); i++) {
            for(int j = 0; j < m.getColumns(); j++) {
                if(m.get(i, j) > 0.0) {
                    binaryMatrix.put(i, j, 1.0);
                }
            }
        }
     ...
     *
     * @param row is in the form [0 0 5 4 2 0 0 3 ... 0 0 1 2 5]
     *
     */
    public static HashMap<Integer, List<Integer>> summarizeRatings(DoubleMatrix row) {

        HashMap<Integer, List<Integer>> response = new HashMap<>(5);
        for(int itemIndex = 0; itemIndex < row.getColumns(); itemIndex++) {

            double value = row.get(0, itemIndex);
